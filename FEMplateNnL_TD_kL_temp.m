...
dT=[900];
Temperi = [300.1]; %% 300.1 is the minmum value due to numerical sensitivity
...
lambdaL=[1200];          %Nondimension lambda = Lambda*Lx^4 / ( E_m*h^3 / ( 12*(1-nu^2) ) )
c_d=0.01;                    % Mass_ratio coefficient (mu/Ma)     
...
disp(3600*(size(lambdaL)*transpose(size(kt))-1))

for kt_index = 1:size(kt')

    k_index = kt(kt_index); %%this is for k rotation
for Lbd_index = 1:size(lambdaL')
    lambda = lambdaL(Lbd_index);
    step = nt(1);
    disp('k_index =')
    disp(k_index)
    disp('lambda =')
...
titstr = ['ND k = ', num2str(k_index), ' , lambda = ', num2str(lambda)];
...
[Ec_300, Em_300, Ent_ROM_300] = MatProp(E_c_vec,E_m_vec,300,k_index,dmlV,temper_en);
[poissc_300, poissm_300, poissnt_ROM_300] = MatProp(nu_c_vec, nu_m_vec,300,k_index,dmlV,temper_en);
[rhoc_300, rhom_300, rhont_ROM_300] = MatProp(Rho_c_vec, Rho_m_vec,300,k_index,dmlV,temper_en);
[alphc_300, alphm_300, alpht_ROM_300] = MatProp(alpha_c_vec, alpha_m_vec,300,k_index,dmlV,temper_en);
...
if ShrCor_en == 0
    Bc_300 = Ec_300/(3*(1-2*poissc_300));
    Bm_300 = Em_300/(3*(1-2*poissm_300));
    Gc_300 = Ec_300./(2*(1.+poissc_300)); Gm_300 =Em_300./(2*(1.+poissm_300));
        
    f1_300 = Gm_300*(9*Bm_300 + 8*Gm_300)/(6*(Bm_300 + 2*Gm_300));

    Bpn_MTS_300 = (Vc./(1+Vm.*((3*Bc_300-3*Bm_300)/(3*Bm_300+4*Gm_300)))) ...
    .*(Bc_300-Bm_300) + Bm_300;
    Gpn_MTS_300 = (Vc./(1+Vm.*((Gc_300-Gm_300)/(Gm_300+f1_300)))) ...
    .*(Gc_300-Gm_300) + Gm_300;

    alph_MTS_300 = (1./Bpn_MTS_300 - 1/Bm_300)/(1/Bc_300 - 1/Bm_300) ...
        .*(alphc_300-alphm_300) + alphm_300;

    for temp = 1 : step+1
        Epn_MTS_300(temp) = 9*Bpn_MTS_300(temp)*Gpn_MTS_300(temp)/(3*Bpn_MTS_300(temp)+Gpn_MTS_300(temp));
        poissn_MTS_300(temp) = (3*Bpn_MTS_300(temp)-2*Gpn_MTS_300(temp))/(2*(3*Bpn_MTS_300(temp)+Gpn_MTS_300(temp)));
    end
    z0n_MTS_300 = (midZ*transpose(Epn_MTS_300))/(sum(Epn_MTS_300));
    z0_MTS_300 = z0n_MTS_300 .* h;
    
    ShrCor_MTS = ShrCor(step, Epn_MTS_300,poissn_MTS_300, z0n_MTS_300,dmlZ);
    ShrCor_ROM = ShrCor(step, Ent_ROM_300, poissnt_ROM_300, z0n_ROM, dmlZ);
end
...
%%%%%%%%%%%%%%%%%%%%%% Boundary condition %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%Depend on BC cccc6x6 = 241 ssss6x6 = 197

%     BC_Simply_Supported; posBC=197;
    BC_Clamped; posBC = 241;
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
 ...
 %-------Mode Shape for thermal post buckling------------------------------    
  ...
  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    D_m300=Em_300*h^3/(12*(1-mean(poissn_MTS)^2)); 
...
if MTS_en ==0
    ...
    Da_PB = diag(sqrt(D1))*(a^2/h)*sqrt((1-mean(poissn_MTS)^2)*rhom_300/Em_300); %Em_300 ??
end
...
title(titstr);
% figure
% plot (dis_w(check_point,:)/h,vel(check_point,:)/h,'LineWidth',2);
% title(titstr);
%--------------------------------------------------------------------------
...
freq = sort(freq);
freq_air = sort(freq_air);
Da  = sort(Da);
Da_air = sort(Da_air);
freq_PB = sort(freq_PB);
Da_PB = sort(Da_PB);


DD1_dTkI(kt_index,Lbd_index) = D1_air(1);
freq_dTkI(kt_index,Lbd_index) = freq(posBC);
freq2_dTkI(kt_index,Lbd_index) = freq(posBC+1);
freqAir_dTkI(kt_index,Lbd_index) = freq_air(posBC);
DDa_dTkI(kt_index,Lbd_index) = Da(posBC);
DDaAir_dTkI(kt_index,Lbd_index) = Da_air(posBC);
freqPB_dTkI(kt_index,Lbd_index) = freq_PB(posBC);
DaPB_dTkI(kt_index,Lbd_index) = Da_PB(posBC);
if time_en
disW_dTkI(kt_index,Lbd_index) = max(abs(dis_w(check_point,end-0.01/t_step:end)/h));
disW2_dTkI(kt_index,Lbd_index) = max(abs(dis_w(check_point,end-0.02/t_step:end)/h));
end
max_wvec_dTkI(kt_index,Lbd_index) = max_w;
%% FFT
if time_en

sample_interv = t_step;
Pren = dis_w(check_point,1:end)/h;
...
if time_en
disWe_dTkI = 100*(disW2_dTkI-disW_dTkI)./disW_dTkI;
end

freqRad_dTkI = freq_dTkI*2*pi;
DaPrak_dTkI = (freqPB_dTkI*2*pi).^2 * a^4 * (rhom_300*h/D_m300);
