...
dT=[0]; dT_interv = 0.1; delta_T = dT(1);
Temperi = [350];
...
lambda=2200;          %Nondimension lambda = Lambda*Lx^4 / ( E_m*h^3 / ( 12*(1-nu^2) ) )
...
K_global=K_elastic - delta_T*K_thermal + Lambda*A_F + N1_s + N2_s + N2_st +1/2*N1_t + 1/3*N2_t;
...
for t1=1:tot_step                           % time routine
    ...
    while (error_norm>=1.0e-6)
        ...
        Tangential_stiffness=zeros(tot_dof,tot_dof);
        Tangential_stiffness=K_elastic - delta_T*K_thermal + Lambda*A_F + N1_s + N2_s + 2*N2_st +N1_t + N2_t + a0*M_global + a1*C_global;
...
figure
plot (tm,dis_w(check_point,:)/h)         %displacement at (x/Lx=3/4)
figure
plot (dis_w(check_point,:)/h,vel(check_point,:)/h)
%--------------------------------------------------------------------------
