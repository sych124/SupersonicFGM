...
F_init = [ F_amp*zeros(44,1);F_amp*ones(11,1);F_amp*zeros(NDof-66,1);F_amp*ones(11,1);F_amp*zeros(0,1)]; % foc SS  [ zeros(NDof/3,1);F_amp*ones(NDof/3,1);zeros(NDof/3,1)];  for CC %
F_t = [F_init.*ones(NDof,int64(F_term/time_interv)) zeros(NDof,int64(time_end/time_interv)-int64(F_term/time_interv))];
deform_time = newmark(mass(activeDof,activeDof), 0*mass(activeDof,activeDof), (stiffness(activeDof,activeDof)-K_T(activeDof,activeDof)), F_t,time_range,init_def, init_vel);
