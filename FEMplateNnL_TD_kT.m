...
ti=0;                       % initial time
tf=0.1;                      % final time
t_step=1/10000;              % the size of time step    
...
%Depend on BC cccc6x6 = 241
posBC = 241;
DD1_dTkI(kt_index,dT_index) = D1_air(1);
freq_dTkI(kt_index,dT_index) = freq(posBC);
freqAir_dTkI(kt_index,dT_index) = freq_air(posBC);
DDa_dTkI(kt_index,dT_index) = Da(posBC);
DDaAir_dTkI(kt_index,dT_index) = Da_air(posBC);
freqPB_dTkI(kt_index,dT_index) = freq_PB(posBC);
DaPB_dTkI(kt_index,dT_index) = Da_PB(posBC);
disW_dTkI(kt_index,dT_index) = max(abs(dis_w(check_point,end-0.01/t_step:end)/h));
disW4_dTkI(kt_index,dT_index) = max(abs(dis_w(check_point,end-0.04/t_step:end)/h));
...
disWe_dTkI = 100*(disW4_dTkI-disW_dTkI)./disW_dTkI;
toc
