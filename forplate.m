...
dT=[0];
Temperi = [600];
Tempere = [600];  %% Temperature of Tc
Tm0 = 300; %% Temperature difference T_ref and Tm
Temper_interv = 0.01;
temper_en = 1;
Temper_factor = 0;
dT=[Temperi(1)];
...
for Lratio = Lratioi(1):Lratio_interv:Lratioe(1)
...
Vc = (dmlV+1/2).^k; Vm = 1-Vc; % Volume Fraction
...
z0_ROM = z0n_ROM .* h;
ShrCor_ROM =  5/6;%ShrCor(step, En_ROM, poissn_ROM, z0n_ROM,dmlZ);
...
if ROMtoMTS ==0
    Da = diag(sqrt(D1))*(a^2/h)*sqrt((1-mean(poissn_ROM)^2)*rhom_300/Em_300);
else
    Da = diag(sqrt(D1))*(a^2/h)*sqrt((1-mean(poissn_MTS)^2)*rhom_300/Em_300);
end
